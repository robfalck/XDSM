...
class XDSM(object):
    ...
    def addComp(self, name, style, string, stack=False, faded=False):
        self.inds[name] = len(self.comps)
        self.comps.append([name, style, string, stack, faded])

    def addDep(self, out, inp, style, string, stack=False, faded=False):
        self.deps.append([out, inp, style, string, stack, faded])

    def getCmds(self):
        def write(i, j, name, style, string, stack, faded):
            M[i][j] = '    \\node'
            str_style = style
            str_style += ', stack' if stack else ''
            str_style += ', faded' if faded else ''
            M[i][j] += ' [ {0}]'.format(str_style)
            M[i][j] += ' (' + name + ')'
            M[i][j] += ' {' + string + '};'
            M[i][j] += ' &\n' if j < n-1 else (' \\\\\n    %Row ' + str(i+2) + '\n')
            print(M[i][j])
        ...
        for name, style, string, stack, faded in self.comps:
            names[inds[name]][inds[name]] = name
        for out, inp, style, string, stack, faded in self.deps:
            names[inds[inp]][inds[out]] = out+'-'+inp

        M = [ [ ('    &\n' if j<n-1 else '    \\\\\n')  for j in range(n) ] for i in range(n) ]
        print(self.comps)
        for name, style, string, stack, faded in self.comps:
            write(inds[name], inds[name], name, style, string, stack, faded)
        for out, inp, style, string, stack, faded in self.deps:
            write(inds[inp], inds[out], out+'-'+inp, style, string, stack, faded)
